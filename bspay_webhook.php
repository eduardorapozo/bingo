...
try {
    ...
    switch ($data['type']) {
        ...
        case 'Withdrawal':
            ...
            error_log(date('Y-m-d H:i:s') . " - Debug: Processando withdrawal - transaction_id: '{$transaction_id}', status: '{$status}'" . PHP_EOL, 3, 'error_codexpay_webhook.log');

            if ($transaction_id && $status) {
                // Buscar saque pelo transaction_id_api
                $stmt = $pdo->prepare("SELECT id, usuario_id, valor, status, transaction_id_api FROM saques WHERE transaction_id_api = ? AND status = 'PENDING'");
                $stmt->execute([$transaction_id]);
                $saque_db = $stmt->fetch(PDO::FETCH_ASSOC);
                
                error_log(date('Y-m-d H:i:s') . " - Debug: Busca saque - transaction_id procurado: '{$transaction_id}', saque encontrado: " . ($saque_db ? "ID {$saque_db['id']}, transaction_id_api: '{$saque_db['transaction_id_api']}'" : "NENHUM") . PHP_EOL, 3, 'error_codexpay_webhook.log');
                ...
                if ($saque_db) {
                    ...
                    error_log(date('Y-m-d H:i:s') . " - Debug: Atualizando saque ID {$saque_db['id']} - novo status: '{$status_saque}'" . PHP_EOL, 3, 'error_codexpay_webhook.log');
                    ...
                    $rows_updated = $stmt_update_saque->execute([$status_saque, $saque_db['id']]);
                    
                    error_log(date('Y-m-d H:i:s') . " - Debug: Update executado - rows affected: " . $stmt_update_saque->rowCount() . PHP_EOL, 3, 'error_codexpay_webhook.log');
