...
// Buscar histórico de depósitos do usuário logado na tabela 'depositos'
$stmt_depositos = $pdo->prepare("SELECT valor, status, data_criacao FROM depositos WHERE usuario_id = ? ORDER BY data_criacao DESC");
$stmt_depositos->execute([$user_data['id']]);
$historico_depositos = $stmt_depositos->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Histórico de Depósitos - Bingo Online</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        /* --- HISTORICO DEPOSITOS CONTAINER - TEMA CLARO --- */
        .historico-container { 
            max-width: 900px; 
            margin: 20px auto; 
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="historico-container">
            <h3><i class="fas fa-history"></i> Histórico de Depósitos</h3>
            
            <?php if (empty($historico_depositos)): ?>
                <div class="empty-state">
                    <i class="fas fa-exclamation-triangle"></i>
                    <p>Você ainda não realizou nenhum depósito.</p>
                </div>
            <?php else: ?>
                <table class="historico-tabela">
                    ...
                    <tbody>
                        <?php foreach ($historico_depositos as $deposito): ?>
                            <tr>
                                <td data-label="Data">
                                    <?php echo (new DateTime($deposito['data_criacao']))->format('d/m/Y H:i'); ?>
                                </td>
                                <td data-label="Valor" class="valor">
                                    + R$ <?php echo number_format($deposito['valor'], 2, ',', '.'); ?>
                                </td>
    ...
    <script>
        ...
        window.onload = function() {
            // Remove o saldo que estava no mobile-header por padrão (apenas para o caso de algum elemento antigo permanecer)
